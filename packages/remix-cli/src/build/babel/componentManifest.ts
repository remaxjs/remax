...
import { HostComponent } from '@alipay/remix-types';
import { kebabCase } from 'lodash';
import { registerNativeComponent } from '@alipay/remix-macro';
...
function getRemixHostComponentName(path: NodePath<t.JSXElement>) {
  const node = path.node;
  const openingElement = node.openingElement;
  ...
  return {
    visitor: {
      JSXElement: (path: NodePath<t.JSXElement>, state: any) => {
        ...
        let name: any = getRemixHostComponentName(path);
