...
export interface SwiperProps {
  ...
  // false 否 是否显示面板指示点 1.0.0
  indicatorColor?: string;
  // rgba(0, 0, 0, .3) 否 指示点颜色 1.1.0
  ...
  // false 否 是否跳过未显示的滑块布局，设为 true 可优化复杂情况下的滑动性能，但会丢失隐藏状态滑块的布局信息 1.9.0
  skipHiddenItemLayout?: boolean;
  // "default" 否 指定 swiper 切换缓动动画类型 2.6.5
  easingFunction?: string;
  // 否 current 改变时会触发 change 事件，event.detail = {current, source} 1.0.0
  onChange?: (event: any) => void;
  // 否 swiper-item 的位置发生改变时会触发 transition 事件，event.detail = {dx: dx, dy: dy} 2.4.3
  onTransition?: (event: any) => void;
  // 否 动画结束时会触发 animationfinish 事件，event.detail 同上 1.9.0
