...
import { render, View } from '../../../src/adapters/toutiao';
...
describe('toutiao', () => {
  describe('remax render', () => {
    ...
    it('create proxy for onClick callback', () => {
      ...
      const handleFocus = () => void 0;
      class Page extends React.Component {
        render() {
          return (
            <View ref={view} onClick={handleClick} onFocus={handleFocus} />
          );
        }
      }
      const container = new Container(p);
      ...
      const newHandleFocus = findFn('onFocus');

      expect(newHandleClick).not.toBe(handleClick);
      expect(newHandleFocus).toBe(handleFocus);
    });
  });
});
