...
export default () => ({
  visitor: {
    ExportDefaultDeclaration: (path: NodePath<t.ExportDefaultDeclaration>, state: any) => {
      ...
      if (state.shouldShip || entries.pages.indexOf(path.hub.file.opts.filename) === -1) {
        return;
      }
      if (t.isExpression(path.node.declaration)) {
        ...
        const createId = addNamed(path, 'createPageConfig', 'remax/lib/adapters/h5');
        const [exportPath] = path.insertAfter(t.exportDefaultDeclaration(t.callExpression(createId, [pageId])));
