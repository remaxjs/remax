...
- **toutiao:** 修复 swiper 的 onAnimationFinish 类型 ([#1024](https://code.alipay.com/remix/remix/issues/1024)) ([e35ce48](https://code.alipay.com/remix/remix/commit/e35ce4833c1c43cc520ffb99a293b7e343871557))
...
- **wechat:** 修复 hideKeyboard 没导出的问题 ([#980](https://code.alipay.com/remix/remix/issues/980)) ([c9afd73](https://code.alipay.com/remix/remix/commit/c9afd73b8eee5ec3150606ca5b8c3d63f64d9362))
...
- **toutiao:** 修复 Canvas[canvasId] 无效的问题 ([#960](https://code.alipay.com/remix/remix/issues/960)) ([ef5c162](https://code.alipay.com/remix/remix/commit/ef5c162a42ced8a89e16e8e0c0681fb15718b95c))
- **toutiao:** 修正 Input 属性默认值 ([#961](https://code.alipay.com/remix/remix/issues/961)) ([b0bd005](https://code.alipay.com/remix/remix/commit/b0bd005d0042735d4413924adddeff92441454c9))
- **toutiao:** 修正一部分新的 api 没有导出的问题 ([#962](https://code.alipay.com/remix/remix/issues/962)) ([42d5d4d](https://code.alipay.com/remix/remix/commit/42d5d4d4516ba746e9610bcb577bfcbe4dcada5a))
...
- **toutiao:** 修正没有导出 getMenuButtonBoundingClientRect 的问题 ([a10f289](https://code.alipay.com/remix/remix/commit/a10f289243c2c82dd356dfc4146bf93cbf232f03))
...
# [2.0.0](https://code.alipay.com/remix/remix/compare/v1...v2.0.0) (2020-05-05)

[2.0 迁移指南](https://remixjs.org/guide/v2-migration)

### Features

- 支持 Web 应用，[文档](https://remixjs.org/guide/one/web)。
- 支持通过 `postcss.config.js` 文件来配置 PostCSS，[文档](https://remixjs.org/guide/config/postcss)。
- 支持自动识别 CSS Modules，[文档](https://remixjs.org/guide/framework/style#css-modules)。
- 支持插件，[文档](https://remixjs.org/guide/advanced/plugin)。
- `remix/one` 支持的全平台生命周期，[文档](https://remixjs.org/api/remix-one/lifecycle)
  - onShow
  - onHide
  - onPullDownRefresh
  - onReachBottom
...
- 修复自定义组件中 usingComponent 无法 resolve node_modules 的问题。[#762](https://code.alipay.com/remix/remix/issues/762)

### Breaking Changes

- 构建工具从 Rollup 切换到 Webpack。
- 废弃 `remix-cli`，现在只要 `remix` 一个依赖即可。
- `alipay` 平台重命名为 `ali`。
- remix.config.js
  - 去掉 rollupOptions (改为 configWebpack，用于修改 webpack 配置)。
  - 去掉 postcss，cssModules，alias 项。
- App 去掉以纯 class 定义 App 的方式
- 废弃 `useShow` `useHide` 等生命周期 hook，请该用 `useAppEvent` 和 `usePageEvent`，[文档](https://remixjs.org/api/remix-macro#useappeventeventname-callback)。
- 去掉 Sass/Less/Stylus 等 CSS 预处理器的默认支持，请通过通过插件引入，[文档](https://remixjs.org/guide/advanced/plugin)
...
- 通过 `remix/macro` 引入的 `requirePlugin`, `requirePluginComponent` 使用小程序插件，废弃原有方式，[文档](https://remixjs.org/guide/basic/plugin)。
- 去除 native 目录支持，[可以配置 webpack copy 插件达到同样的效果](https://remixjs.org/guide/advanced/hybrid#%E5%8E%9F%E7%94%9F%E6%B7%B7%E5%90%88%E5%BC%80%E5%8F%91)
...
## [1.0 更新日志](https://code.alipay.com/remix/remix/blob/v1/CHANGELOG.md)
